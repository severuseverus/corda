...
class ContractUpgradeFlowTest {
    ...
    fun `2 parties contract upgrade`() {
        ...
        val twoPartyDummyContract = DummyContract.generateInitial(0, notary, a.services.legalIdentity.party.ref(1), b.services.legalIdentity.party.ref(1))
        val signedByA = a.services.signInitialTransaction(twoPartyDummyContract)
        val stx = b.services.addSignature(signedByA)

        a.services.startFlow(FinalityFlow(stx, setOf(a.services.legalIdentity.party, b.services.legalIdentity.party)))
        mockNet.runNetwork()
    ...
    fun `2 parties contract upgrade using RPC`() {
        rpcDriver(initialiseSerialization = false) {
            ...
            val handle = rpcA.startFlow(::FinalityInvoker, stx, setOf(a.services.legalIdentity.party, b.services.legalIdentity.party))
            mockNet.runNetwork()
    ...
    fun `upgrade Cash to v2`() {
        // Create some cash.
        val anonymous = false
        val result = a.services.startFlow(CashIssueFlow(Amount(1000, USD), OpaqueBytes.of(1), a.services.legalIdentity.party, notary, anonymous)).resultFuture
        mockNet.runNetwork()
        ...
        assertEquals(Amount(1000000, USD).`issued by`(a.services.legalIdentity.party.ref(1)), (firstState.state.data as CashV2.State).amount, "Upgraded cash contain the correct amount.")
        assertEquals<Collection<AbstractParty>>(listOf(a.services.legalIdentity.party), (firstState.state.data as CashV2.State).owners, "Upgraded cash belongs to the right owner.")
    }
