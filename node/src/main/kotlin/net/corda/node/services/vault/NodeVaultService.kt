...
class NodeVaultService(private val services: ServiceHub, dataSourceProperties: Properties, databaseProperties: Properties?) : SingletonSerializeAsToken(), VaultService {
    ...
    override fun <T : FungibleAsset<U>, U : Any> tryLockFungibleStatesForSpending(lockId: UUID,
                                                                                  ...
                                                                                  amount: Amount<U>,
                                                                                  contractType: Class<out T>): VaultService.StatesForSpendingResult<T, U> {
        if (amount.quantity == 0L) {
            return VaultService.StatesForSpendingResult(emptyList(), false, amount, amount.copy(0L))
        }

        // This rewrites the query tree to ensure correct soft-locking semantics and UNCONSUMED status
        val queryEditor = object : IQueryCriteriaParser {
            ...
            var modifiedCriteria: QueryCriteria = QueryCriteria.VaultQueryCriteria(contractStateTypes = setOf(contractType),
                    includeSoftlockedStates = false,
                    includeStatesWithLockId = lockId,
        ...
        val results = queryEditor.queryForEligibleStates(eligibleStatesQuery)
        ...
        if (claimedStates.isEmpty() || claimedAmount < amount.quantity) {
            return VaultService.StatesForSpendingResult(claimedStates, false, amount, amount.copy(claimedAmount))
        }
        try {
            softLockReserve(lockId, claimedStates.map { it.ref }.toNonEmptySet())
            return VaultService.StatesForSpendingResult(claimedStates, true, amount, amount.copy(claimedAmount))
        } catch (ex: StatesNotAvailableException) {
            return VaultService.StatesForSpendingResult(emptyList(), false, amount, amount.copy(0L))
        }
    }
